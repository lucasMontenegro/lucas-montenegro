...
import server from "new/local/server"
...
jest.mock(`new/local/server/startHttp`, () => ({ __esModule: true, default: jest.fn() }))
startHttp.mockImplementation((...args) => {
  mockFn(`startHttp`, args)
  return `startHttp`
})
jest.mock(`new/local/server/handleClusterExit`, () => ({
  __esModule: true,
  default: `new/local/server/handleClusterExit`,
}))
